...
export function renderEreChart(ctx, chartInstance, data, E_max_re, thetaCrit) {
    ...
    return new Chart(ctx, {
        type: 'line',
        data: {
            labels: Array.from({length: 89}, (_, i) => i + 1),
            datasets: [{
                label: 'Ere(\u03D1) [kN]',
                data: data,
        ...
        options: {
            ...
            plugins: { title: { display: true, text: 'Earth Force E\u209a\u2090(\u03D1) vs. Sliding Angle \u03D1' }, legend: { display: false } },
            scales: {
                x: { title: { display: true, text: 'Sliding Angle \u03D1 [deg]', color: '#4b5563' }, min: 0, max: 90 },

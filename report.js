...
export async function generatePDFReport(soilLayers) {
    ...
    try {
        ...
        if (applySilo) {
            let siloModelText = document.getElementById('silo_model').options[document.getElementById('silo_model').selectedIndex].text;
            geometryBody.push(
                ['Silo Theory Applied (for sigma_v,max)', `Yes - ${siloModelText}`],
        ...
        if(applySilo) await addImageFromElement(doc, 'silo-details-display-max', pageDetails, { widthFactor: 0.8 }, '2.2 Silo Theory Details', 2);
        await addImageFromElement(doc, 'detailed-parameters', pageDetails, null, applySilo ? '2.3 Detailed Parameters at Critical Angle' : '2.2 Detailed Parameters', 2);
